...
function getCandles(provider, symbol, start, end, candleSize) {
  ...
  const groupedCandles = candles1m
    .reduce((groups, candle1m) => {
      ...
      const isGroupFull = !(candle1m.timestamp < candles1m[0].timestamp + candleSizeInMinutes * 60 * 1000 * groups.length)
  ...
  console.log('groups of candles:')
  groupedCandles.forEach(group => console.log(group.length))
  const newCandles = groupedCandles
    ...
    console.log(newCandles[newCandles.length - 1])
  return newCandles
}
